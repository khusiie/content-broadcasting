import { useState, useEffect, useCallback } from 'react';
import { useParams } from 'react-router-dom';
import { Clock, Tv, AlertCircle, Loader2 } from 'lucide-react';
import contentService from '../../services/content.service';
const LiveDisplay = () => {
  const { teacherId } = useParams();
  const [contentItems, setContentItems] = useState([]);
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(true);
  const fetchLive = useCallback(async () => {
    try {
      const data = await contentService.getLiveContent(teacherId);
      if (Array.isArray(data) && data.length > 0) {
        setContentItems(data);
      } else {
        setContentItems([]);
      }
      setError('');
    } catch (err) {
      setError('System Offline');
    } finally {
      setLoading(false);
    }
  }, [teacherId]);
  useEffect(() => {
    fetchLive();
    const interval = setInterval(fetchLive, 30000);
    return () => clearInterval(interval);
  }, [fetchLive]);
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-black">
        <div className="text-center animate-fade-in">
          <Tv className="animate-pulse text-indigo-500 mx-auto mb-4" size={48} />
          <p className="text-gray-500 text-sm">Loading broadcast...</p>
        </div>
      </div>
    );
  }
  if (error) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-black">
        <div className="text-center animate-fade-in">
          <AlertCircle size={64} className="text-rose-600 mx-auto mb-6" />
          <h1 className="text-3xl font-bold text-rose-500 mb-2">{error}</h1>
          <p className="text-gray-600">Unable to connect to the broadcast service.</p>
        </div>
      </div>
    );
  }
  return (
    <div className="min-h-screen bg-black flex flex-col items-center justify-center p-8 overflow-hidden">
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-indigo-500/10 rounded-full blur-[120px] -z-10" />
      {contentItems.length > 0 ? (
        <div className="w-full max-w-6xl animate-fade-in">
          {contentItems.map((content, index) => (
            <div key={content.id || index} className="flex flex-col items-center text-center mb-16 last:mb-0">
              <div className="relative group">
                <div className="absolute -inset-1 bg-gradient-to-r from-indigo-500 to-purple-600 rounded-3xl blur opacity-25 group-hover:opacity-40 transition duration-1000" />
                <img
                  src={content.file_url}
                  alt={content.title}
                  className="relative rounded-2xl max-h-[60vh] shadow-2xl object-contain border border-white/10"
                  loading="lazy"
                />
              </div>
              <div className="mt-10 space-y-4">
                <span className="px-4 py-2 rounded-full bg-white/5 border border-white/10 text-indigo-400 font-bold uppercase tracking-widest text-sm">
                  {content.subject}
                </span>
                <h1 className="text-5xl md:text-6xl font-black !text-white leading-tight">
                  {content.title}
                </h1>
                <p className="text-xl text-gray-400 max-w-3xl mx-auto">
                  {content.description || 'Educational broadcast content.'}
                </p>
              </div>
            </div>
          ))}
          <div className="mt-12 flex items-center justify-center gap-6 text-gray-500 font-medium">
            <div className="flex items-center gap-2">
              <Clock size={20} />
              <span>Broadcast Active</span>
            </div>
            <div className="w-1 h-1 rounded-full bg-gray-700" />
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-emerald-500 animate-ping" />
              <span>Live • Auto-refresh 30s</span>
            </div>
            <div className="w-1 h-1 rounded-full bg-gray-700" />
            <span>{contentItems.length} active item{contentItems.length > 1 ? 's' : ''}</span>
          </div>
        </div>
      ) : (
        <div className="text-center animate-fade-in">
          <AlertCircle size={64} className="text-gray-700 mx-auto mb-6" />
          <h1 className="text-5xl font-bold text-gray-600 mb-4">No Active Broadcast</h1>
          <p className="text-xl text-gray-700">Please wait for the next scheduled lesson.</p>
          <div className="mt-12 p-6 rounded-2xl bg-white/[0.02] border border-white/5">
            <p className="text-xs text-gray-500 uppercase tracking-widest">Display ID: {teacherId}</p>
          </div>
        </div>
      )}
      <div className="fixed bottom-8 left-8 flex items-center gap-3">
        <div className="w-8 h-8 rounded-lg bg-indigo-500 flex items-center justify-center text-white">
          <Tv size={16} />
        </div>
        <span className="font-bold text-gray-500 tracking-tighter">EDU-CAST</span>
      </div>
    </div>
  );
};
export default LiveDisplay;
